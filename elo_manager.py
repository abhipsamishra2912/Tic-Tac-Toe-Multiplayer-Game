...
def determine_actual_score(result, uid1, uid2):
    if result == "draw":
        return 0.5, 0.5
    
    elif result == f"{uid1}_win":
        return 1.0, 0.0
    
    elif result == f"{uid2}_win":
        return 0.0, 1.0
...
def update_ratings(uid1, uid2, result):
    r1, r2 = get_player_ratings(uid1, uid2)

    E1, E2 = compute_expected_score(r1, r2)

    S1, S2 = determine_actual_score(result, uid1, uid2)
